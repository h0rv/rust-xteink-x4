...
use crate::epub::layout::{FontMetrics, LayoutEngine, Page, TextStyle};
use crate::epub::metadata::{parse_container_xml, parse_opf, EpubMetadata, ManifestItem};
use crate::epub::spine::{create_spine, parse_spine, Spine};
...
fn test_zip_open_sample_epub() {
    ...
    assert!(
        zip.get_entry("META-INF/container.xml").is_some(),
...
fn test_zip_read_container() {
    ...
    assert!(
        ...
        "Should contain container element"
    );
    assert!(
        content.contains("rootfile"),
        "Should contain rootfile element"
    );
    assert!(
        content.contains("EPUB/package.opf"),
        "Should reference package.opf"
    );
}
...
fn test_zip_list_entries() {
    ...
    assert!(
        filenames.contains(&"META-INF/container.xml"),
        "Should have container.xml"
    );
...
fn test_zip_entry_not_found() {
    ...
    assert!(
        zip.get_entry("nonexistent/file.txt").is_none(),
...
fn test_manifest_lookup() {
    ...
    if let Some(item) = cover_item {
        assert!(
            item.href.ends_with(".xhtml") || item.href.ends_with(".html"),
    ...
    assert!(
        metadata.get_item("nonexistent").is_none(),
...
fn test_manifest_items_have_valid_properties() {
    ...
    for item in &metadata.manifest {
        ...
        assert!(
            ...
            "Manifest item should have a media type"
        );
    }
}
...
fn test_tokenize_with_headings() {
    ...
    assert!(
        tokens.iter().any(|t| matches!(t, Token::Heading(1))),
...
fn test_layout_headings() {
    ...
    assert!(
        all_text.contains("Chapter Title"),
        "Should contain heading text"
    );
    assert!(
        all_text.contains("Chapter content"),
...
fn test_epub_full_pipeline() {
    ...
    if let Some(first_chapter_id) = spine.current_id() {
        ...
        let chapter_item = metadata
            .get_item(first_chapter_id)
        ...
        if let Some(chapter_entry) = zip.get_entry(&chapter_path) {
            ...
            let chapter_bytes = zip
                .read_file(&entry, &mut chapter_buf)
...
fn test_read_all_chapters() {
    ...
    assert!(
        chapters_read > 0,
        "Should be able to read at least one chapter"
    );
    assert_eq!(
        chapters_read,
        spine.len(),
        "Should read all chapters in spine"
    );
}
